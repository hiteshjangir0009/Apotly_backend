...
const User_register = Async_handler(async (req, res) => {
    ...
    if (email.includes("@") === false) {
        return res.status(400).json(
            new API_response(400, [], "invalid email")
        )
...
// Add Adopted Pet
const Add_adopted_pet = Async_handler(async (req, res) => {
    const { pet_name, category, breed } = req.body;

    // Check for empty fields
    if ([pet_name, category, breed].some((field) => !field?.trim())) {
        return res.status(400).json(
            new API_response(400, [], "All fields (pet_name, category, breed) are required")
        );
    }

    // Update user's adopted pet information
    ...
    return res.status(200).json(
        new API_response(200, updatedUser, "Adopted pet added successfully")
    );
});
...
export { User_register, User_login, Update_address, Add_adopted_pet }